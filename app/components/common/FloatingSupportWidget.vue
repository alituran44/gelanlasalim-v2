...
function generateBotResponse(userQuery: string) {
  ...
  if (q.includes('ilk ihale') || q.includes('1 ay') || q.includes('deneme') || q.includes('ücretsiz') || q.includes('fiyat') || q.includes('ücret') || q.includes('bedava')) {
    replyText = '🎁 Lansmanımıza özel olarak İLK İHALENİZ tüm alıcı ve tedarikçi firmalarımıza %100 ÜCRETSİZDİR! Kredi kartı bilgisi gerekmez, 0 komisyon ile ilk ihalenizi açabilir ve teklif verebilirsiniz.'
    actionLink = { text: 'İlk İhalenizi Ücretsiz Başlatın →', url: '/abonelik' }
    ...
    replyText = `Sorunuz için teşekkürler! İhaleciBurada platformunda lansmana özel ilk ihale ücretsiz avantajı ile ihale açabilir, kurumsal teklif toplayabilir veya ihale ve satın alma yapabilirsiniz. Daha detaylı canlı bilgi için WhatsApp hattımıza da yazabilirsiniz.`
  ...
  <div class="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3 font-sans select-none">
    ...
    <transition name="chat-popup">
      ...
      >
        ...
        <!-- First Tender Free Promo Strip inside Chat -->
        <div class="bg-gradient-to-r from-emerald-600 to-teal-700 text-white px-3.5 py-1.5 text-[10px] font-bold flex items-center justify-between shadow-inner">
          <span class="flex items-center gap-1.5">
            ...
            <span>Lansmana Özel: İlk İhale %100 Ücretsiz</span>
          </span>
        ...
        <div class="px-3 py-2 bg-white border-t border-slate-100 flex gap-1.5 overflow-x-auto text-[10px] font-bold text-slate-600 scrollbar-none">
          <button 
            @click="sendPreset('İlk İhale Ücretsiz kampanyası nasıl başlatılır?')"
