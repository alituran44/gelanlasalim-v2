  ...
  <div class="min-h-screen bg-[#F4F6F9] text-slate-800 font-sans text-xs flex flex-col">
    ...
    <div class="max-w-[1440px] w-full mx-auto px-4 sm:px-6 py-4">
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <!-- Mobil Filtre Aç / Kapat Butonu (lg:hidden) -->
        <div class="lg:hidden w-full col-span-1">
          <button 
            type="button"
          ...
          >
            <span class="flex items-center gap-2">
              ...
              <span>Filtreler & Kategoriler</span>
              <span v-if="selectedCategory !== 'Tümü' || selectedCity !== 'Tümü' || selectedAuthority !== 'Tümü' || selectedSector !== 'Tümü' || selectedCompany !== 'Tümü' || filterType !== 'Tümü' || filterKeyword" class="px-2 py-0.5 rounded-full bg-[#0084B4] text-white text-[10px] font-black">
                Aktif Filtre
              </span>
            </span>
            <div class="flex items-center gap-1 text-[11px] text-slate-500 font-semibold">
              <span>{{ showMobileFilters ? 'Gizle' : 'Göster' }}</span>
              <ChevronDown :size="14" class="transition-transform duration-200" :class="showMobileFilters ? 'rotate-180 text-[#0084B4]' : ''" />
            </div>
          </button>
        </div>
        ...
        <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-6 pb-6 border-b border-slate-800/60">
          <div class="space-y-3 max-w-3xl">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sky-500/10 border border-sky-500/30 text-sky-400 text-[11px] font-bold uppercase tracking-wider">
              ...
              <span>B2B Entegrasyon Altyapısı</span>
            </div>
            ...
            <p class="text-xs sm:text-sm text-slate-400 font-medium leading-relaxed">
              Kurumsal kaynak planlama ve harici veri kaynaklarınızı İhaleciBurada B2B portalına bağlayın. Manuel ihale açma ve teklif aktarım süreçlerini otomatikleştirin.
            </p>
          </div>
          ...
          <div class="flex items-center gap-3">
            ...
            >
              <span>Entegrasyon Yönetim Paneli</span>
              <ArrowRight :size="14" />
            </NuxtLink>
          </div>
        </div>

        <!-- Sadece 4 Temel Entegrasyon Kartı -->
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          
          <!-- 1. SAP Entegrasyonu -->
          <NuxtLink
            to="/entegrasyonlar?tab=sap"
            class="group bg-[#0d1322]/90 hover:bg-[#11182c] border border-slate-800 hover:border-sky-500/50 rounded-2xl p-6 flex flex-col justify-between transition-all duration-200 shadow-md cursor-pointer block"
          >
            <div class="space-y-4">
              <div class="flex items-center justify-between">
                <div class="w-11 h-11 rounded-xl bg-sky-500/10 border border-sky-500/20 flex items-center justify-center text-sky-400 group-hover:scale-105 transition-transform">
                  <Cpu :size="22" />
                </div>
                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold font-mono uppercase tracking-wider bg-sky-500/10 border border-sky-500/30 text-sky-400">
                  CONNECTED
                </span>
              </div>
              <div>
                ...
                <p class="text-[11px] text-sky-400/90 font-mono mt-1 font-bold">S/4HANA · ECC 6.0 · OData</p>
              </div>
              <p class="text-xs text-slate-400 leading-relaxed">
                SAP sisteminizdeki onaylanmış PR satın alma taleplerini otomatik çekerek canlı ihaleye dönüştürün; sonuçlanan teklifleri ve satıcı faturalarını SAP sisteminize aktarın.
              </p>
            </div>
            <div class="pt-4 mt-6 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400 group-hover:text-sky-400 font-bold transition">
              <span>Kurulum & Simülasyon</span>
          ...
          >
            <div class="space-y-4">
              <div class="flex items-center justify-between">
                <div class="w-11 h-11 rounded-xl bg-teal-500/10 border border-teal-500/20 flex items-center justify-center text-teal-400 group-hover:scale-105 transition-transform">
                  <Database :size="22" />
                </div>
                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold font-mono uppercase tracking-wider bg-teal-500/10 border border-teal-500/30 text-teal-400">
                  CANLI SYNC
                </span>
              </div>
              <div>
                <h3 class="text-base sm:text-lg font-black text-white group-hover:text-teal-300 transition">
                  Logo & Netsis Sync
                </h3>
                <p class="text-[11px] text-teal-400/90 font-mono mt-1 font-bold">Tiger 3 · Netsis 3 · REST Servis</p>
              </div>
              <p class="text-xs text-slate-400 leading-relaxed">
                Logo ve Netsis stok kartları, cari hesaplar, sipariş fişleri ve ambar girişleriyle anlık iki yönlü veri eşitleme ve otomatik faturalandırma.
              </p>
            </div>
            <div class="pt-4 mt-6 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400 group-hover:text-teal-400 font-bold transition">
              <span>Bağlantı & Eşitleme</span>
              <ArrowRight :size="14" class="group-hover:translate-x-1 transition-transform" />
            </div>
          </NuxtLink>
          ...
          >
            <div class="space-y-4">
              <div class="flex items-center justify-between">
                <div class="w-11 h-11 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400 group-hover:scale-105 transition-transform">
                  <Network :size="22" />
                </div>
                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold font-mono uppercase tracking-wider bg-blue-500/10 border border-blue-500/30 text-blue-400">
                  API v2.0
                </span>
              </div>
              <div>
                ...
                <p class="text-[11px] text-blue-400/90 font-mono mt-1 font-bold">Business Central · Finance · Dataverse</p>
              </div>
              <p class="text-xs text-slate-400 leading-relaxed">
                Tedarik zinciri, onay hiyerarşileri ve maliyet merkezi mutabakatı; ihale sözleşmelerini Microsoft Dynamics kayıtlarına doğrudan bağlama.
              </p>
            </div>
            <div class="pt-4 mt-6 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400 group-hover:text-blue-400 font-bold transition">
              <span>Yetkilendirme & Ayar</span>
              <ArrowRight :size="14" class="group-hover:translate-x-1 transition-transform" />
            </div>
          </NuxtLink>
          ...
          >
            <div class="space-y-4">
              <div class="flex items-center justify-between">
                <div class="w-11 h-11 rounded-xl bg-purple-500/10 border border-purple-500/20 flex items-center justify-center text-purple-400 group-hover:scale-105 transition-transform">
                  <FileSpreadsheet :size="22" />
                </div>
                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold font-mono uppercase tracking-wider bg-purple-500/10 border border-purple-500/30 text-purple-400">
                  OPENAPI & XLS
                </span>
              </div>
              <div>
                <h3 class="text-base sm:text-lg font-black text-white group-hover:text-purple-300 transition">
                  REST API & Excel
                </h3>
                <p class="text-[11px] text-purple-400/90 font-mono mt-1 font-bold">OpenAPI 3.0 · Webhooks · .xlsx Parser</p>
              </div>
              <p class="text-xs text-slate-400 leading-relaxed">
                Özel yazılımlar ve CRM sistemleri için token tabanlı JSON REST API veya on binlerce kalemlik Excel malzeme listesini tek tıkla yükleme.
              </p>
            </div>
            <div class="pt-4 mt-6 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400 group-hover:text-purple-400 font-bold transition">
              <span>API Dokümanı & Excel</span>
              <ArrowRight :size="14" class="group-hover:translate-x-1 transition-transform" />
            </div>
          </NuxtLink>
