...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const newTender: TenderItem = {
      ...
      awardMode: body.awardMode || 'ALL_OR_NOTHING',
      kalemler: Array.isArray(body.kalemler) ? body.kalemler : [],
      lotlar: Array.isArray(body.lotlar) ? body.lotlar : [],
      currency: body.currency || 'TRY',
      vatType: body.vatType || 'vat_included',
      deliveryLocation: body.deliveryLocation || body.city || 'Türkiye',
      deliveryDuration: body.deliveryDuration || body.sure || '7 gün kaldı',
      paymentTerms: body.paymentTerms || 'pesin',
      startPrice: typeof body.startPrice === 'number' ? body.startPrice : undefined,
      reservePrice: typeof body.reservePrice === 'number' ? body.reservePrice : undefined,
      minStep: typeof body.minStep === 'number' ? body.minStep : 1000,
      requiresParticipationApproval: Boolean(body.requiresParticipationApproval),
      minBidsCount: typeof body.minBidsCount === 'number' ? body.minBidsCount : 1,
      specVersion: typeof body.specVersion === 'number' ? body.specVersion : 1,
      parentTenderId: body.parentTenderId || undefined,
